...
### Accuracy and Response Times
In Fig 1 (Experiment 2) it is visible that the motion discrimination performance is around or even above chance when the prior stimulus was congruent with the motion, e.g. a counter-clockwise yaw rotation followed by a left translation. Direction discrimination performance for the second motion is even better when a yaw rotation is followed by an incongruent inertial translation. In this Experiment there are no clear MAEs. As for experiment 1, a more detailed analysis for accuracy and reaction times can be found in the supplementary materials. 
...
### Accuracy and Response Times
In Fig 1 (Experiment 3) it is again visible that the motion discrimination performance is below chance when the prior stimulus was in the same direction, e.g. a left translation follwed by a counter-clockwise yaw rotation. Direction discrimination performance is above chance level when a yaw rotation is preceeded by an incongruent or fore-aft motion. As in experiment 1, the response patterns indicate MAEs, except for ISIs of 600ms. Again, correct response times are faster for incongruent trials (left translation followed by clockwise rotation or right translation followed by counter-clockwise rotation) than for congruent trials (left translation followed by counter-clockwise rotation or right translation followed by counter-clockwise rotation), while the opposite pattern is found for incorrect response times. A more detailed analysis for accuracy and reaction times can be found in the supplementary materials. 
...
## Experiment 1
To describe data from experiment 1 in more detail, a Bayesian multilevel logistic regression for accuracy with treatment coded factors Cue Congruence (congruent, incongruent, neutral) and ISI (50ms, 100ms, 200ms, 600ms) was calculated. For the reaction times a Bayesian multilevel regression with a ex-Gaussian likelihood function and factors accuracy (correct, wrong), Cue Congruence (congruent, incongruent, neutral) and ISI (50ms, 100ms, 200ms, 600ms) was computed. Priors were set to default in brms.

## Experiment 3
To describe data from experiment 2 and 3 in more detail, for each experiment a Bayesian multilevel logistic regression for accuracy with treatment coded factors Cue Congruence (congruent, incongruent) and ISI (50ms, 100ms, 200ms, 600ms) was calculated. For the reaction times a Bayesian multilevel regression with a ex-Gaussian likelihood function and factors accuracy (correct, wrong), Cue Congruence (congruent, incongruent) and ISI (50ms, 100ms, 200ms, 600ms) was computed. Priors were set to default in brms.
